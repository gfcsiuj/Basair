            ...
            <div className="bg-bg-primary rounded-xl w-full max-w-sm max-h-[70vh] shadow-xl flex flex-col animate-scaleIn" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {filteredItems.map(item => (
                         <div key={item.id} onClick={() => { onSelect(item.id); onClose(); }} className="flex items-center w-full px-4 py-3 hover:bg-bg-secondary transition-colors group cursor-pointer">
                            <div className="flex-1 text-center">
                                <span className={`${selectedId === item.id ? 'font-bold text-primary' : 'text-text-primary'}`}>
                                    {item.name}
                                </span>
                            </div>
                            <button
